...
func Trust() {
	...
	for _, info := range imageInfo {
		...
		imageIds[k] = info.ImageId
	}

	for _, cntr := range containers {
		if cntr.Image == "registry:2" || cntr.Image == "mysql:8" {
			continue
		}

		if _, ok := imageIds[cntr.Image]; !ok {
			log.Debugf("image[%v] is not in imageIds", cntr.Image)
			if err := cli.ContainerStop(context.Background(), cntr.ID, nil); err != nil { // TODO timeout
				log.Warnf("ContainerStop: id=%v %v", cntr.ID, err)
			}
			/*
				if err := cli.ContainerRemove(context.Background(), cntr.ID, types.ContainerRemoveOptions{}); err != nil {
					log.Warnf("ContainerRemove: id=%v %v", cntr.ID, err)
				}

				if _, err := cli.ImageRemove(context.Background(), cntr.ImageID, types.ImageRemoveOptions{}); err != nil {
					log.Warnf("ContainerRemove: id=%v %v", cntr.ID, err)
				}
			*/
		}
